...
2. 在确认窗口内，如果动作键仍然按着，程序会记录出现过的最新非基础层；动作键之后新按下的单层，只有在 `layerTakeoverWindowMs` 内才可以接管更早按住的单层。
...
| `layerTakeoverWindowMs` | 35 | 新按下肩键/扳机向前接管动作键的回看窗口。保持它短于 `actionLayerGraceMs`，可以避免上一枚动作键被新 R1/RB 抢走。 |
